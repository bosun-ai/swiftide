    ...
    async fn next_completion(&self) -> Result<Option<Vec<ChatMessage>>> {
        ...
        if history[current..].is_empty()
            || (self.stop_on_assistant
                && matches!(history.last(), Some(ChatMessage::Assistant(_, _, _)))
...
mod tests {
    ...
    async fn test_filters_messages_before_summary() {
        let messages = vec![
            ...
            ChatMessage::User("Hello".into()),
            ChatMessage::Assistant(Some("Hello there".into()), None, None),
    ...
    async fn test_filters_messages_before_summary_with_assistant_last() {
        ...
        assert!(matches!(new_messages[2], ChatMessage::Assistant(_, _, _)));
        ...
        let messages = vec![
            ...
            ChatMessage::Assistant(Some("Hello there again".into()), None, None),
        ];
        ...
        assert_eq!(
            new_messages[3],
            ChatMessage::Assistant(Some("Hello there again".to_string()), None, None)
        );
