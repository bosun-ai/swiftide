...
- *(core)* Add pipeline statistics collection for monitoring and observability ([#156](https://github.com/bosun-ai/swiftide/issues/156))

  - Add `StatsCollector` for thread-safe statistics collection
  - Add `PipelineStats` struct with node counts, token usage, and timing
  - Integrate statistics into `Pipeline::run()` method
  - Add `Pipeline::stats()` method to retrieve current statistics
  - Support real-time statistics access during pipeline execution

### Added

- New `swiftide_core::statistics` module with:
  - `PipelineStats` - Statistics snapshot with nodes_processed, nodes_failed, nodes_stored, transformations_applied, token_usage per model
  - `StatsCollector` - Thread-safe collector with atomics and mutex
  - `ModelUsage` - Token usage tracking per model (prompt_tokens, completion_tokens, total_tokens, request_count)
