...
impl<Input: NodeArg + Clone, Output: NodeArg + Clone> Task<Input, Output> {
    ...
    ///
    /// ```no_run
    /// use swiftide_tasks::{ConcurrencyModel, Task};
    ///
    ...
    /// use swiftide_tasks::{DynNodeId, NodeError, Task, TaskNode};
    ///
    /// #[derive(Clone)]
    /// struct Increment;
    ///
    ...
    /// - a closure returning a typed [`MarkedTransition`](crate::MarkedTransition)
    /// - a closure returning a raw [`Transition`] for advanced control flow
    /// - a [`JoinTarget`](crate::JoinTarget) built from a join node
    /// - a mapped join target produced by [`JoinTarget::map`](crate::JoinTarget::map)
    ///
    /// # Examples
    ///
    /// ```no_run
