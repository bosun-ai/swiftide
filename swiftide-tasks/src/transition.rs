...
//!
//! ```no_run
//! use swiftide_tasks::{JoinInput, NodeError, Task, Transition};
//!
...
///
/// Build a `JoinTarget` from a join node with [`NodeId::join`](crate::NodeId::join),
/// and then register it through
/// [`Task::register_transition`](crate::Task::register_transition).
...
/// [`NodeId::join_any`](crate::NodeId::join_any).
#[must_use]
...
///     |input: &swiftide_tasks::JoinInput<i32>| -> Result<i32, NodeError> {
///         Ok(input.iter().copied().sum())
///     },
/// );
///
...
///         .and(&right, value)
///         .join_with(join.join())
///         .concurrency_model(swiftide_tasks::ConcurrencyModel::Parallel)
/// })?;
/// task.register_transition(left, join.join())?;
/// task.register_transition(right, join.join())?;
...
/// [`Task::register_transition`](crate::Task::register_transition) closures.
#[must_use]
...
impl Transition {
    ...
    ///
    /// ```no_run
    /// use swiftide_tasks::{NodeError, Task, Transition};
    ///
    /// let mut task = Task::<i32, i32>::new();
...
impl AnyJoinInput {
    ...
    ///
    /// ```no_run
    /// use swiftide_tasks::{AnyJoinInput, NodeError, Task, Transition};
    ///
    /// let mut task = Task::<i32, i32>::new();
    /// let start = task.register_node_fn(|input: &i32| -> Result<i32, NodeError> { Ok(*input) });
    /// let left = task.register_node_fn(|input: &i32| -> Result<i32, NodeError> { Ok(*input + 1) });
