...
/// A typed handle to a registered node in a [`Task`](crate::Task).
///
...
/// [`Transition::fan_out`](crate::Transition::fan_out) when building static fan-out
...
impl<T: TaskNode + ?Sized> NodeId<T> {
    ...
    ///
    /// ```no_run
    /// use swiftide_tasks::{NodeError, Task};
    ///
    /// let mut task = Task::<i32, i32>::new();
    ...
    T: TaskNode<Input = AnyJoinInput> + ?Sized,
{
    ...
    ///
    /// Prefer [`NodeId::join`] with [`JoinInput<T>`](crate::JoinInput) when all branches
...
impl<T: TaskNode + 'static + ?Sized> NodeId<T> {
    ...
    ///
    /// Prefer [`Task::register_node`](crate::Task::register_node) when building normal task
